CREATE DEFINER=`admin`@`%` PROCEDURE `table_workers_3_approve_weld`(
...
IF (key_stage="4") THEN
	...
	PREPARE stmt FROM @sql_text;EXECUTE stmt;DEALLOCATE PREPARE stmt;
    SET @sql_text = concat('UPDATE workers.',@worker_table_name,' SET approveDate= \"',now(),'\" WHERE JobID=',@key_JobID,';');
	PREPARE stmt FROM @sql_text;EXECUTE stmt;DEALLOCATE PREPARE stmt;

    IF @key_fitter_2_id IS NULL THEN
